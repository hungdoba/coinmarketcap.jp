...
export const headCells: CoinListTableHeadCell[] = [
  ...
  {
    id: 'priceChangePercentage24HInCurrency',
    numeric: true,
    label: '24h %',
    customisable: true,
  },
  {
    id: 'priceChangePercentage7DInCurrency',
    numeric: true,
    label: '7d %',
    customisable: true,
  },
...
const CoinListTableHeader: React.FC<CoinListTableHeaderProps> = ({
  order,
  orderBy,
  onRequestSort,
}) => {
  ...
  const idToSort: (property: keyof Coin) => CoinSortingKey = (
    property: keyof Coin
  ) => {
    switch (property) {
      case 'marketCap':
  ...
  const sortToId: (property: CoinSortingKey) => keyof Coin = (
  ...
  return (
    <TableHead className={classes.root}>
      <TableRow>
        {headCells
          .filter(
            (headCell: CoinListTableHeadCell) =>
              coinList.coinListTableColumns.includes(headCell.id) ||
              !headCell.customisable
          )
          .map((headCell: CoinListTableHeadCell, index: number) => (
            ...
            >
              <TableSortLabel
                disabled={
                  headCell.id !== 'marketCap' && headCell.id !== 'totalVolume'
                }
