...
const CoinDataCardGroup: React.FC = () => {
  ...
  const circulatingSupply =
    coinDetails.value?.marketData.circulatingSupply || 0;
      ...
      {coinDetails.value && coinDetails.status !== 'LOADING' ? (
        <Box display="flex" justifyContent="space-between" padding={3}>
            ...
            <Box height="100%" padding="18px 24px">
              ...
              <Box display="flex" alignItems="center">
                ...
                <Typography
                  variant="body1"
                  color="textSecondary"
                  className={classes.marketCapRank}
                >
          ...
          <Paper className={classes.detailsSurface}>
            <Box height="100%" padding="18px 24px">
              <Typography variant="h6" className={classes.gutterBottom} noWrap>
                {formatNumber(roundDecimals(circulatingSupply, 0))}{' '}
                {coinDetails.value.symbol.toUpperCase()}
              </Typography>
              <Box
                display="flex"
                alignItems="center"
                className={classes.supplyCard}
              >
                <Typography variant="body1" color="textSecondary" noWrap>
                  Circulating Supply
                </Typography>
          ...
          <Paper className={classes.detailsSurface}>
            <Box height="100%" padding="18px 24px">
              <Typography variant="h6" className={classes.gutterBottom} noWrap>
                {maxSupply ? formatNumber(roundDecimals(maxSupply, 0)) : '∞'}{' '}
                {coinDetails.value.symbol.toUpperCase()}
              </Typography>
              <Box
                display="flex"
